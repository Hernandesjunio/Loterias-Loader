...
{
    [Fact]
    public async Task H2_retry_after_429_is_capped_to_remaining_budget()
    {
        ...
        var client = CreateClient(http, runContext, delay, clock);

        await Assert.ThrowsAsync<HttpRequestException>(() =>
            client.GetLatestContestIdAsync(LoteriaModalityKeys.Lotofacil, CancellationToken.None));
    ...
    [Fact]
    public async Task H2_retry_skipped_immediately_when_budget_is_zero()
    {
        var t0 = ContractTestHarness.Utc("2026-07-12T20:00:00Z");
        var clock = new FakeClock(t0);
        var delay = new FakeDelay(clock);
